...
function BS_Uni_Sys(couple_func)
    function BS_Uni_Sys(X, p, t)
        K, a2, a4, r, Eps = p
        dX = zeros(6)
        dX[1:3] = single_Bick(X[1:3], [K, a2, a4, r], t) + Eps * [couple_func(p) for p in  X[1:3] - X[4:6]]
        dX[4:6] = single_Bick(X[4:6], [-K, a2, a4, r], t) + Eps * [couple_func(p) for p in  X[4:6] - X[1:3]]
        return SVector{6}(dX)
    end
    return BS_Uni_Sys
end